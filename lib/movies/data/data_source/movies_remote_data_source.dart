...
try {
  ...
  @override
  Future<Either<Failures,List<MovieModel>>>  getPopularMovies(int pageNumber) async{
    try {
      final response =  await dio.get(Constants.getSpecificPagePopularPath(pageNumber));
      if(response.data['success']== false){
        return left(ServerFailure.fromSuccessBody(response.data));
      }else {
        return right((response.data['results'] as List)
  ...
  @override
  Future<Either<Failures,List<MovieModel>>>  getTopRatedMovies(int pageNumber) async{
    try {
      final response =  await dio.get(Constants.getSpecificPageTopRatedPath(pageNumber));
      if(response.data['success']== false){
        return left(ServerFailure.fromSuccessBody(response.data));
      }else {
        return right((response.data['results'] as List)
  ...
  @override
  Future<Either<Failures, List<MovieModel>>> getMovieRecommendations(int movieID) async{
    try {
      final response =  await dio.get(Constants.getMovieRecommendationsPath(movieID));
      if(response.data['success']== false){
  ...
  @override
  Future<Either<Failures, MovieDetailsModel>> getMovieDetails(int movieID) async{
